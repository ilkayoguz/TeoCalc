...
public static partial class T6xCardFormat
{
    ...
    return new T6xDocument
    {
      ...
      Profile = profile
        ?? (string.IsNullOrWhiteSpace(document.Profile)
          ? NormalizeTargetCpu(document.Model)
          : document.Profile),
      Category = document.Category,
      Title = document.Title,
      Description = document.Description,
    ...
    TeoCardDocument? metadata = null)
  {
    ...
    return new T6xDocument
    {
      ...
      SchemaVersion = T6xDocument.CurrentSchemaVersion,
      TargetCpu = "T-67",
      Profile = string.IsNullOrWhiteSpace(metadata?.Profile)
        ? (metadata is null ? "T-67" : NormalizeTargetCpu(metadata.Model))
        : metadata.Profile,
      ...
      CodeEncoding = CardCodeEncoding.Normalize(metadata?.Program.CodeEncoding),
