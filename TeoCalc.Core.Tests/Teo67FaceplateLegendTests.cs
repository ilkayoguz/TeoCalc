...
public sealed class Teo67FaceplateLegendTests
{
  ...
  public void EnterRow_NotHp65PrefixStkRegPrgm()
  {
    ...
    Assert.IsTrue(ClassicFaceplateGlyphs.UsesCardSlotExchangeLabel("P\u2194S"));
    Assert.IsTrue(ClassicFaceplateGlyphs.UsesCardSlotExchangeLabel("x\u2194y"));
    Assert.IsFalse(ClassicFaceplateGlyphs.IsPlainArialSkirtLabel("P\u2194S"));
    // CapBelow P↔S chevrons share the same card-slot glyph height / connector as CapFace x↔y.
    const float fontSize = 14f;
    float connector = ClassicFaceplateGlyphs.MeasureCardSlotExchangeConnector(fontSize);
    Assert.AreEqual(
      CardSlotExchangeArt.MeasureWidth(ClassicFaceplateGlyphs.CardSlotExchangeGlyphHeight(fontSize)),
      connector);
