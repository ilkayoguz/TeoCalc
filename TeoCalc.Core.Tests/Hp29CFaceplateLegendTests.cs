...
[TestClass]
public sealed class Hp29CFaceplateLegendTests
{
  private static ProgramVocabulary LoadVocabulary() =>
    ProgramVocabulary.Load(TeoCalcPaths.ResourcePath("Engine/HP-29/Program/program.vocabulary.json"));
  ...
  public void PrimaryLabels_MatchPrintedKeyboard()
  {
    ...
    foreach ((int index, string label) in expected)
    {
      Assert.AreEqual(
        label,
        CalcFaceplateLayout.LabelForKey(vocabulary.KeyChart[index], vocabulary, "Woodstock", "HP-29"),
        $"Index {index}");
    }

    Assert.AreNotEqual(
      "BST",
      CalcFaceplateLayout.LabelForKey(vocabulary.KeyChart[1], vocabulary, "Woodstock", "HP-29"));
  }
  ...
  {
    ProgramVocabulary vocabulary = LoadVocabulary();
    Assert.AreEqual("e", vocabulary.KeyChart[9].Char);
    Assert.AreEqual(
      "\u03a3+",
      CalcFaceplateLayout.LabelForKey(vocabulary.KeyChart[9], vocabulary, "Woodstock", "HP-29"));
    Assert.AreEqual(" ", vocabulary.KeyChart[33].Char);
    Assert.AreEqual(
      "R/S",
      CalcFaceplateLayout.LabelForKey(vocabulary.KeyChart[33], vocabulary, "Woodstock", "HP-29"));
    Assert.AreNotEqual(
      "\u03a3+",
      CalcFaceplateLayout.LabelForKey(vocabulary.KeyChart[33], vocabulary, "Woodstock", "HP-29"));
  }
  ...
  [TestMethod]
  public void KeyColors_GoldAndBluePrefix_BlackRows_WhitePad()
  {
    Assert.AreEqual(CalcButtonStyle.Orange, CalcFaceplateKeyStyles.StyleForKey("Woodstock", "HP-29", 3));
    Assert.AreEqual(CalcButtonStyle.Blue, CalcFaceplateKeyStyles.StyleForKey("Woodstock", "HP-29", 4));
    Assert.AreEqual(CalcButtonStyle.DarkGrey, CalcFaceplateKeyStyles.StyleForKey("Woodstock", "HP-29", 0));
    Assert.AreEqual(CalcButtonStyle.DarkGrey, CalcFaceplateKeyStyles.StyleForKey("Woodstock", "HP-29", 9));
    Assert.AreEqual(CalcButtonStyle.DarkGrey, CalcFaceplateKeyStyles.StyleForKey("Woodstock", "HP-29", 10));
    Assert.AreEqual(CalcButtonStyle.DarkGrey, CalcFaceplateKeyStyles.StyleForKey("Woodstock", "HP-29", 12));
    Assert.AreEqual(CalcButtonStyle.White, CalcFaceplateKeyStyles.StyleForKey("Woodstock", "HP-29", 16));
    Assert.AreEqual(CalcButtonStyle.White, CalcFaceplateKeyStyles.StyleForKey("Woodstock", "HP-29", 33));

    Assert.AreEqual(CalcButtonStyle.Orange, CalcFaceplateKeyStyles.StyleForKey("Woodstock", "HP-29C", 3));
    Assert.AreEqual(CalcButtonStyle.Blue, CalcFaceplateKeyStyles.StyleForKey("Woodstock", "HP-29C", 4));
    Assert.AreEqual(CalcButtonStyle.DarkGrey, CalcFaceplateKeyStyles.StyleForKey("Woodstock", "HP-29C", 0));
    Assert.AreEqual("T-29C", CalcModelIds.ToProductLabel("HP-29C"));
    Assert.AreEqual("T-29C", CalcModelCatalog.Resolve("HP-29C").ProductLabel);
  }

  [TestMethod]
  public void ShiftPreview_FramesGoldFAndBlueG()
  {
    Assert.AreEqual(3, ShiftPreviewController.IndicatorKeyIndex(ShiftPreviewMode.Gold, "Woodstock", "HP-29"));
    Assert.AreEqual(4, ShiftPreviewController.IndicatorKeyIndex(ShiftPreviewMode.Blue, "Woodstock", "HP-29"));

    ShiftPreviewController preview = new();
    preview.HandleKeyPress(3, "Woodstock", "HP-29");
    Assert.AreEqual(ShiftPreviewMode.Gold, preview.Mode);
    preview.HandleKeyPress(4, "Woodstock", "HP-29");
