...
public static class CardSlotLabelArt
{
  ...
  private static string AssetsRoot => Path.Combine(TeoCalcPaths.ResourcePath("Engine/HP-65/Assets"), "CardSlot");

  public static bool IsReady =>
    Hp65FaceplateSvgAssets.CanDrawCardSlotLabels && Labels.All(label => File.Exists(PathFor(label.File)));

  public static HpClassicFaceplateGlyphs.LabelSize Measure(int column, float fontSize)
  {
  ...
  {
    if ((uint)column >= Labels.Length || !Hp65FaceplateSvgAssets.CanDrawCardSlotLabels)
    {
      return false;
    }
    ...
    HpClassicFaceplateGlyphs.LabelSize size = Measure(column, fontSize);
    ...
    return Hp65FaceplateSvgAssets.TryDrawSvg(draw, min, max, path, rasterW, rasterH);
  }
