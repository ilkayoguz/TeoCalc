"""Build A4.png: cyan keypad, anchored equal-gap keys, mirrored yellow footer."""
...
def keypad_pocket_mask(a0: np.ndarray, y_bottom: int) -> np.ndarray:
    ...
    h, _w = a0.shape[:2]
    mask = np.zeros((h, a0.shape[1]), dtype=bool)
    ref_left, ref_right = 46, 426
    for y in range(PANEL_TOP, min(y_bottom, h - 1) + 1):
        row_l, row_r = ref_left, ref_right
        if left[y] >= 0 and right[y] > left[y]:
            row_l, row_r = int(left[y]), int(right[y])
        mask[y, row_l : row_r + 1] = True
    return mask


def mirror_left_footer_curve(bottom: np.ndarray, x0: int, x1: int) -> np.ndarray:
    out = bottom.copy()
    cx = (x0 + x1) // 2
...
def build(a2_path: Path = A2, a0_path: Path = A0, out_path: Path = OUT) -> None:
    ...
    report = layout.row_layout_report(keys)
    keypad_bottom = 832

    pocket = keypad_pocket_mask(a0, keypad_bottom)
    out[pocket] = PANEL_CYAN
    for key in keys:
        out[key.y : key.y + key.h, key.x : key.x + key.w] = KEY_RED
    ...
    print("Row layout (margins L/R, gaps px, y, x1, xN_right):")
    for row, info in sorted(report.items()):
        gaps = info["gaps"]
        gap_str = ",".join(str(g) for g in gaps) if gaps else "-"
        print(
            f"  row {row + 1}: margin {info['margin_left']}/{info['margin_right']} "
            f"gaps=[{gap_str}] y={info['y']} x1={info['first_x']} "
            f"xN_r={info['last_right']}"
        )
...
if __name__ == "__main__":
    out_name = sys.argv[1] if len(sys.argv) > 1 else "A4.png"
