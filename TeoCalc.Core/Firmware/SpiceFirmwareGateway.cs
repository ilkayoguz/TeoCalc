...
/// <summary>
/// Native Spice-family firmware life-cycle (power / key / batch / display).
/// Mirrors Panamatik HPSpice Headless* timing without calling Panamatik at runtime.
/// </summary>
public sealed class SpiceFirmwareGateway : ICalcFirmwareGateway
{
  ...
  public SpiceCpu? Cpu { get; private set; }
  ...
  public void AttachCpu(SpiceCpu? cpu)
  {
    Cpu = cpu;
    PowerOn = false;
    _runAccumulator = 0f;
  ...
  public void PowerOff()
  {
    ...
    {
      Cpu.Reset();
      Cpu.State.Flags &= ~SpiceCpuFlags.DisplayOn;
    }
  ...
  public bool IsDisplayVisible() =>
    PowerOn
    && Cpu is not null
    && (Cpu.State.Flags & SpiceCpuFlags.DisplayOn) != 0
  ...
  private void RunInstructionBatch(int steps)
  {
    ...
    LastBatch = new FirmwareBatchSnapshot(
      ...
      Grp: (byte)((Cpu.State.Flags & SpiceCpuFlags.Bank) != 0 ? 1 : 0),
      P: Cpu.State.P,
      Classic: null);
  ...
  private void RefreshDisplayFromCpu()
  {
    ...
    string text = SpiceFirmwareDisplay.BuildLedText(Cpu.State);
    bool blankPulse = PowerOn && (Cpu.State.Flags & SpiceCpuFlags.DisplayOn) == 0;
