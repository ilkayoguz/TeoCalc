...
/// </summary>
public sealed class Teo01Cpu : CpuBase
{
  ...
  private readonly ITeo01ToneSink _tones;
  ...
  public Teo01Cpu(IMicrocodeRom rom, MicrocodeHandlerCatalog handlers, ITeo01ToneSink? tones = null)
    : base(rom, handlers)
  {
    _dispatchTable = Teo01DispatchTable.Build();
    State = new Teo01CpuState();
    _tones = tones ?? NullTeo01ToneSink.Instance;
  }

  public Teo01CpuState State { get; }
  ...
  {
    State.Flags &= ~Teo01CpuFlags.DisplayOn;
    _refreshDisplay = true;
    _pendingKeycode = keyCode;
    State.KeyBuffer = keyCode;
    State.ExtraFlags &= ~Teo01ExtraFlags.Sleep;
    Running = true;
  }
  ...
  public override MicrocodeHandlerEntry Step()
  {
    ...
    if ((State.Flags & Teo01CpuFlags.Carry) != 0)
    {
      State.Flags &= ~Teo01CpuFlags.Carry;
      State.Flags |= Teo01CpuFlags.PrevCarry;
    }
    else
    {
      State.Flags &= ~Teo01CpuFlags.PrevCarry;
    ...
    Execute(handler.OpcodeAlias, opcode);
    State.LastOpcode = opcode;
    State.LastHandlerId = handler.HandlerId;
    StepCount++;

    if ((State.ExtraFlags & Teo01ExtraFlags.Sleep) != 0)
    {
  ...
  {
    if ((State.ExtraFlags & Teo01ExtraFlags.Sleep) != 0 && _refreshDisplay)
    {
      _refreshDisplay = false;
    }

    if ((State.ExtraFlags & Teo01ExtraFlags.Blink) != 0 && ++_blinkCnt >= 20)
    {
      _blinkCnt = 0;
      State.Flags ^= Teo01CpuFlags.DisplayOn;
      _refreshDisplay = true;
    ...
    if ((State.ExtraFlags & Teo01ExtraFlags.Wakeup) != 0)
    {
      State.ExtraFlags &= ~Teo01ExtraFlags.Wakeup;
      State.ExtraFlags &= ~Teo01ExtraFlags.Sleep;
      State.KeyBuffer = 63;
      Running = true;
    }

    if (_displayCnt != 0 && --_displayCnt == 0 && (State.ExtraFlags & Teo01ExtraFlags.DisplayOnHold) == 0)
    {
      State.Flags &= ~Teo01CpuFlags.DisplayOn;
      _refreshDisplay = true;
    }
  ...
  public void ApplyPendingKeyDisplayHold()
  {
    ...
    if ((State.ExtraFlags & Teo01ExtraFlags.DisplayOnHold) == 0)
    {
      ...
      if (State.C[11] == 2 || (State.ExtraFlags & Teo01ExtraFlags.Scwp) != 0)
      {
        _displayCnt = 0;
      }
    }
  ...
  private void Execute(string alias, ushort opcode)
  {
    switch (alias)
    {
      ...
      case "op_display_off":
        State.Flags &= ~Teo01CpuFlags.DisplayOn;
        State.ExtraFlags &= ~Teo01ExtraFlags.Blink;
        break;
      case "op_blink":
        State.ExtraFlags |= Teo01ExtraFlags.Blink;
        break;
      case "op_enscwp":
        State.ExtraFlags |= Teo01ExtraFlags.Scwp;
        break;
      case "op_dsscwp":
        State.ExtraFlags &= ~Teo01ExtraFlags.Scwp;
        break;
      case "op_swinc":
        State.ExtraFlags &= ~Teo01ExtraFlags.SwDec;
        break;
      case "op_swdec":
        State.ExtraFlags |= Teo01ExtraFlags.SwDec;
        break;
      case "op_swstrt":
        ...
        State.ExtraFlags |= Teo01ExtraFlags.SwStarted;
        break;
      case "op_swstop":
        _stopwatch.Stop();
        State.ExtraFlags &= ~Teo01ExtraFlags.SwStarted;
        break;
      case "op_altog":
        State.ExtraFlags ^= Teo01ExtraFlags.AlarmActive;
        break;
      case "op_cdex":
      ...
      case "op_ctom":
        ...
        break;
      case "op_ftoap":
        if (State.P < Teo01CpuState.WordSize)
        {
  ...
  {
    if ((State.Flags & Teo01CpuFlags.PrevCarry) == 0)
    {
      State.ProgramCounter = (ushort)((State.Rom << 8) | (opcode >> 2));
    }
  }
  ...
  {
    if ((State.ExtraFlags & Teo01ExtraFlags.AlarmActive) != 0)
    {
  ...
  private bool Hp01Clinc()
  {
    ...
    if ((++State.TickCnt & 0x1F) == 0)
    {
      ...
      if (digit != _second)
      {
        _second = digit;
        Hp01IncCl();
        if ((State.ExtraFlags & Teo01ExtraFlags.Scwp) != 0)
        {
          State.ExtraFlags |= Teo01ExtraFlags.Wakeup;
          refresh = true;
        }
        ...
        if ((State.ExtraFlags & Teo01ExtraFlags.AlarmActive) != 0 && Hp01CheckAlarm())
        {
          State.ExtraFlags &= ~Teo01ExtraFlags.AlarmActive;
          State.ExtraFlags |= Teo01ExtraFlags.Blink;
          _tones.Alarm();
    ...
    if ((State.ExtraFlags & Teo01ExtraFlags.SwStarted) != 0)
    {
      long ticks = _stopwatch.ElapsedMilliseconds / 10;
      ticks = (State.ExtraFlags & Teo01ExtraFlags.SwDec) != 0
        ? _swStartTime - ticks
        : ticks + _swStartTime;
      if (ticks < 0)
      {
  ...
  private void RegAdd()
  {
    for (byte i = _first; i <= _last; i++)
    {
      ...
      byte sum = (byte)(_dest![i] + addend + (((State.Flags & Teo01CpuFlags.Carry) != 0) ? 1 : 0));
      if (sum >= 10)
      {
        sum -= 10;
        State.Flags |= Teo01CpuFlags.Carry;
  ...
  private void RegSub()
  {
    for (byte i = _first; i <= _last; i++)
    {
      ...
      sbyte diff = (sbyte)(left - right - (((State.Flags & Teo01CpuFlags.Carry) != 0) ? 1 : 0));
      if (diff < 0)
      {
        diff += 10;
        State.Flags |= Teo01CpuFlags.Carry;
