...
/// <summary>Woodstock-family CPU stepper (HP-25 reference; shared by HP-21/22/27/29).</summary>
public sealed class WoodstockCpu
{
  ...
  private readonly WoodstockMicrocodeRom _rom;
  private readonly MicrocodeHandlerCatalog _handlers;
  private bool _programMode;

  public WoodstockCpu(WoodstockMicrocodeRom rom, MicrocodeHandlerCatalog handlers)
  {
    _rom = rom;
    _handlers = handlers;
    State = new WoodstockCpuState();
  }

  public WoodstockCpuState State { get; }
  ...
  public MicrocodeHandlerEntry Step()
  {
    ...
    while (State.InstructionState != WoodstockInstructionState.Norm);
  ...
  private MicrocodeHandlerEntry ExecuteCycle()
  {
    ...
    if ((State.Flags & WoodstockCpuFlags.Carry) != 0)
    {
      State.Flags |= WoodstockCpuFlags.PrevCarry;
    }
    else
    {
      State.Flags &= ~WoodstockCpuFlags.PrevCarry;
    ...
    {
      case WoodstockInstructionState.Norm:
        alias = ResolveNormAlias(opcode);
        Execute(alias, opcode);
        break;
      case WoodstockInstructionState.Branch:
        alias = "op_branch_target";
        State.InstructionState = WoodstockInstructionState.Norm;
        if ((State.Flags & WoodstockCpuFlags.PrevCarry) == 0)
        {
          State.ProgramCounter = (ushort)((State.ProgramCounter & 0xFC00) | opcode);
        }

        break;
      case WoodstockInstructionState.SelfTest:
        alias = "op_rom_selftest";
        if (opcode == 1060)
        {
          State.Flags ^= WoodstockCpuFlags.Bank;
        }
    ...
    if ((opcode & 0x3D0) != 464)
    {
      State.Flags &= ~WoodstockCpuFlags.PCarry;
    }
  ...
  private void Execute(string alias, ushort opcode)
  {
    switch (alias)
    {
      ...
      case "op_return":
        ...
        break;
      case "op_arith":
        WoodstockCpuArithmetic.Execute(opcode, State);
        break;
      case "op_set_s":
      ...
      case "op_del_sel_rom":
        ...
        State.Flags |= WoodstockCpuFlags.DelRom;
        break;
      case "op_mx":
        WoodstockCpuStack.Mx(State.Registers, opcode);
        break;
      case "op_a_to_rom_addr":
        ...
        break;
      case "op_y_to_a":
        WoodstockCpuStack.YToA(State.Registers);
        break;
      case "op_register_to_c":
        WoodstockCpuDataRam.RegisterToCOpcode(State, opcode);
        break;
      case "op_c_to_register":
        WoodstockCpuDataRam.CToRegisterOpcode(State, opcode);
        break;
      case "op_c_to_addr":
        WoodstockCpuDataRam.CToAddress(State);
        break;
      case "op_c_to_data":
        WoodstockCpuDataRam.CToData(State);
        break;
      case "op_clear_data_regs":
        WoodstockCpuDataRam.ClearDataRegs(State);
        break;
      case "op_c_to_stack":
        WoodstockCpuStack.CToStack(State.Registers);
        break;
      case "op_stack_to_a":
        WoodstockCpuStack.StackToA(State.Registers);
        break;
      case "op_down_rotate":
        WoodstockCpuStack.DownRotate(State.Registers);
        break;
      case "op_clear_reg":
        WoodstockCpuStack.ClearRegisters(State.Registers);
        break;
      case "op_set_p":
      ...
      case "op_keys_to_a":
        ...
        break;
      case "op_display_off":
        State.Flags &= ~WoodstockCpuFlags.DisplayOn;
        break;
      ...
      case "op_rom_selftest":
        State.InstructionState = WoodstockInstructionState.SelfTest;
  ...
  private void TestPNe(ushort opcode)
  {
    ...
    if ((State.Flags & WoodstockCpuFlags.PCarry) != 0 && State.P == 1 && expected == 0)
    {
      State.Flags |= WoodstockCpuFlags.Carry;
    }
  ...
  {
    if ((State.Flags & WoodstockCpuFlags.DelRom) == 0)
    {
      return;
    }
