...
internal static class WoodstockCpuDataRam
{
  public static void RegisterToC(WoodstockCpuState state, byte addr)
  {
    if (addr < 64)
    {
      for (byte i = 0; i < 7; i++)
  ...
  public static void CToRegister(WoodstockCpuState state, byte addr)
  {
    if (addr >= 64)
    {
      return;
  ...
  public static void CToAddress(WoodstockCpuState state) =>
    state.RamAddress = (byte)((state.Registers.C[1] << 4) + state.Registers.C[0]);

  public static void CToData(WoodstockCpuState state) =>
    CToRegister(state, state.RamAddress);

  public static void CToRegisterOpcode(WoodstockCpuState state, ushort opcode)
  {
    state.RamAddress = (byte)((state.RamAddress & 0xF0) + (opcode >> 6));
    CToData(state);
  }

  public static void RegisterToCOpcode(WoodstockCpuState state, ushort opcode)
  {
    if (opcode >> 6 != 0)
    {
      state.RamAddress = (byte)((state.RamAddress & 0xF0) + (opcode >> 6));
  ...
  public static void ClearDataRegs(WoodstockCpuState state)
  {
